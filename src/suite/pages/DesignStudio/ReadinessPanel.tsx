  ...
  return (
    <div className="rp">
      <div className="rp__head">
        <div className={`rp-ring rp-ring--${tone}`} role="img" aria-label={`Manufacturing readiness ${score} percent`}>
        ...
        <div className="rp__headtext">
          <span className="rp__eyebrow">Manufacturing Readiness</span>
          <b>{score === 100 ? 'Production ready' : score >= 70 ? 'Almost there' : 'Getting started'}</b>
          <small>
            {ready.length}/{readiness.checks.length} specs complete
          </small>
        </div>
      </div>
      ...
      {missing.length > 0 && (
        <div className="rp__group">
          <span className="rp__label">Needs attention</span>
          {missing.map((c) => (
            <button
              key={c.id}
              type="button"
              ...
              onClick={() => onFix?.(c.id)}
              disabled={!onFix}
              title={onFix ? `Fix: ${c.label}` : undefined}
            >
              ...
              {onFix && <span className="rp-item__cta">Fix</span>}
            </button>
          ))}
        </div>
      )}
      ...
      <div className="rp__group">
        <span className="rp__label">Ready</span>
        {ready.map((c) => (
