...
export function CreativeDirector({ suggestions, onApply, onDismiss, onClose }: Props) {
  ...
  return (
    <aside className="cd" role="complementary" aria-label="Creative director suggestions">
      <div className="cd__head">
        <span className="cd__title">
          <IcoSparkle width="14" height="14" />
          Creative Director
        </span>
        <button type="button" className="cd__close" aria-label="Dismiss all suggestions" onClick={onClose}>
        ...
        {suggestions.map((s) => (
          <li key={s.id} className="cd__item">
            ...
            <div className="cd__actions">
              <button type="button" className="cd__skip" onClick={() => onDismiss(s.id)}>Not now</button>
